...
// mode=1 performs staging copies only, making it a real barrier/copy test.
...
static bool InitSession12(ID3D12Device *dev)
{
    ...
    wchar_t data_path[MAX_PATH] = {};
    GetModuleFileNameW(g_self, data_path, MAX_PATH);
    if (wchar_t *s = wcsrchr(data_path, L'\\'))
        *(s + 1) = L'\0';

    NVSDK_NGX_Result r = NVSDK_NGX_D3D12_Init(
        0x1000000ULL, data_path, g12.dev, nullptr, NVSDK_NGX_Version_API);
    ...
    if (NVSDK_NGX_FAILED(r))
    {
        r = NVSDK_NGX_D3D12_Init_with_ProjectID(
            ...
            data_path, g12.dev, nullptr, NVSDK_NGX_Version_API);
...
static bool MakeOwnedTexture12(int slot, UINT w, UINT h, DXGI_FORMAT fmt, D3D12_RESOURCE_FLAGS flags)
{
    D3D12_HEAP_PROPERTIES hp = {};
    hp.Type = D3D12_HEAP_TYPE_DEFAULT;
    ...
    HRESULT hr = g12.dev->CreateCommittedResource(
        ...
        nullptr, __uuidof(ID3D12Resource), reinterpret_cast<void **>(&g12.tex[slot]));

    if (FAILED(hr) || !g12.tex[slot])
    {
        Log("[feed12] CreateCommittedResource(%s %ux%u %s flags=0x%X) failed 0x%08X",
            kSlotName[slot], w, h, FormatName(fmt), (unsigned)flags, hr);
        return false;
    }

    Log("[feed12] owned %-6s %ux%u %s flags=0x%X",
        kSlotName[slot], w, h, FormatName(fmt), (unsigned)flags);
    return true;
}

static bool BuildResources12(ID3D12GraphicsCommandList *list, UINT w, UINT h, DXGI_FORMAT bb_fmt)
{
    if (!list || !g12.dev) return false;
    ReleaseFrameResources12();
    ...
    {
        Warn("native D3D12: target %s requires output %s; format-conversion copy-back is not implemented yet",
             FormatName(g12.color_fmt), FormatName(g12.output_fmt));
        return false;
    }

    bool ok =
        MakeOwnedTexture12(SLOT_COLOR,  w, h, g12.color_fmt, D3D12_RESOURCE_FLAG_NONE) &&
        MakeOwnedTexture12(SLOT_OUTPUT, w, h, g12.output_fmt, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS) &&
        MakeOwnedTexture12(SLOT_DEPTH,  w, h, DXGI_FORMAT_R32_FLOAT, D3D12_RESOURCE_FLAG_NONE) &&
        MakeOwnedTexture12(SLOT_MV,     w, h, DXGI_FORMAT_R16G16_FLOAT, D3D12_RESOURCE_FLAG_NONE);
    ...
    {
        if (ccode)
            Warn("native D3D12 CreateFeature exception 0x%08X", ccode);
        else
...
static bool StageInputs12(ID3D12GraphicsCommandList *list,
                          ID3D12Resource *color,
                          ID3D12Resource *mv,
                          ID3D12Resource *depth)
{
    if (!list || !color || !mv || !depth) return false;

    BarrierOn(list, color,
              D3D12_RESOURCE_STATE_RENDER_TARGET,
              D3D12_RESOURCE_STATE_COPY_SOURCE);
    BarrierOn(list, mv,
              D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
              D3D12_RESOURCE_STATE_COPY_SOURCE);
    BarrierOn(list, depth,
              D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
              D3D12_RESOURCE_STATE_COPY_SOURCE);
    ...
    list->CopyResource(g12.tex[SLOT_COLOR], color);
    list->CopyResource(g12.tex[SLOT_MV], mv);
    list->CopyResource(g12.tex[SLOT_DEPTH], depth);
                           ...
                           reshade::api::resource_view rtv)
{
        ...
        dd.Format != DXGI_FORMAT_R32_FLOAT)
    {
        ...
        {
            said = true;
            Log("[feed12] input mismatch: color %llux%u %s flags=0x%X | mv %llux%u %s flags=0x%X | depth %llux%u %s flags=0x%X",
                ...
                (unsigned long long)dd.Width, dd.Height, FormatName(dd.Format), (unsigned)dd.Flags);
        }
        return;
    }
    ...
    NVSDK_NGX_D3D12_DLSS_Eval_Params ep = {};
    ep.Feature.pInColor = g12.tex[SLOT_COLOR];
    ep.Feature.pInOutput = g12.tex[SLOT_OUTPUT];
    ep.Feature.InSharpness = 0.0f;
    ep.pInDepth = g12.tex[SLOT_DEPTH];
    ep.pInMotionVectors = g12.tex[SLOT_MV];
    ...
    {
        if (ecode)
            Log("[feed12] evaluate exception 0x%08X", ecode);
        else
            Log("[feed12] evaluate failed 0x%08X (%s)", re, NgxResultName(re));

        BarrierOn(list, g12.tex[SLOT_OUTPUT], D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON);
        BarrierOn(list, g12.tex[SLOT_DEPTH], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COMMON);
...
if (eval_index <= 10)
    ...
    list->CopyResource(color, g12.tex[SLOT_OUTPUT]);
